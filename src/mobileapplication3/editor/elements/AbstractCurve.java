...
public abstract class AbstractCurve extends Element {
    ...
    public void paint(Graphics g, int zoomOut, int offsetX, int offsetY, boolean drawThickness) {
        ...
        for (int i = 0; i < pointsCache.getSize() - 1; i++) {
            ...
            g.drawLine(xToPX(startPoint[0], zoomOut, offsetX), yToPX(startPoint[1], zoomOut, offsetY), xToPX(endPoint[0], zoomOut, offsetX), yToPX(endPoint[1], zoomOut, offsetY), LINE_THICKNESS, zoomOut, drawThickness, true, true);
            startPoint = endPoint;
        }
    }
